...
function Category({categoryUpdates}) {
...
const videoDrop = async (e , categoryDetails)=>{
  ...
  const videDetails = JSON.parse(e.dataTransfer.getData("videoDetails"))
  console.log(videDetails);


  

  // categoryDetails.allVideos.push(videDetails)
  // console.log(categoryDetails);

  if(categoryDetails.allVideos.find((item)=>item.id== videDetails.id)){
    toast.error('Video already in the same category')
...
const videoDrag = (e ,videoDetails ,categoryDetails)=>{
  ...
  useEffect(() => {
    getCategory()
  }, [categoryStatus ,deleteStatus, categoryUpdateStatus , categoryUpdates])


  return (
    <>
      <h5 className='mt-5 '>Category</h5>
      <button onClick={handleShow} className="btn btn-warning mt-4 w-100">Category</button>
      {allCategory?.length > 0 ? allCategory?.map((item) => (
        ...
        <div className="border border-secondary p-3 rounded mt-4" droppable onDragOver={(e)=>videoOver(e)} onDrop={(e)=>videoDrop(e,item)}>
         ...
         {item?.allVideos.length>0 && item?.allVideos.map((video)=>(
              <div draggable onDragStart={(e)=>videoDrag(e, video,item)}  >
                <Videocard videDetails = {video} present ={true} />
              </div>
         ))
          }
        </div>
        ...
        :
        <h6 className='text-center text-warning mt-4'>Category Not Added Yet ...</h6>}
